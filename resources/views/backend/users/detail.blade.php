        ...
        <div class="row">
            ...
            <div class="mb-3 col-6">
                <label name="username" class="form-label">
                    @if (is_null($user->first_name))
                        {{ __('web.not-data') }} @else{{ $user->first_name }}
                    @endif
                </label>
            </div>
            <!-- Apellidos -->
            ...
            <div class="mb-3 col-6">
                <label name="username" class="form-label">
                    @if (is_null($user->last_name))
                        {{ __('web.not-data') }} @else{{ $user->last_name }}
                    @endif
                </label>
            </div>
            <!-- Dirección -->
            ...
            <div class="mb-3 col-6">
                <label name="username" class="form-label">
                    @if (is_null($user->address))
                        {{ __('web.not-data') }} @else{{ $user->address }}
                    @endif
                </label>
            </div>
            <!-- Ciudad -->
            ...
            <div class="mb-3 col-6">
                @if (is_null($user->cities))
                    {{ __('web.not-data') }}
                @else
                        <label name="username" class="form-label">{{ $user->cities->name }}</label>
                @endif
            </div>
            <!-- País -->
            <div class="mb-3 col-6 vvv">
            ...
            <div class="mb-3 col-6">
                @if (is_null($user->city_id))
                    {{ __('web.not-data') }}
                @else
                        <label name="username" class="form-label mb-5">{{ $user->cities->countries->name }}</label>
                @endif
            </div>
        </div>
    </div>
    ...
    <div class="container border border-dark d-flex justify-content-center rounded-bottom shadow-lg p-3 bg-white">
        <div class="row">
            <div class="mb-3 mt-3 col-12">
                <a class="btn btn-dark btn-lg" href="{{ route('profiles.edit', ['profile' => $user->id]) }}">
